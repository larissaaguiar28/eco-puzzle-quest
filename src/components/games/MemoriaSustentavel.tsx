...
// 1. Memoized Fireflies (Evita re-render e Hydration Mismatch)
...
// 2. Componente de Carta isolado com Acessibilidade
const MemoryCard = ({ card, onFlip }: { card: CardType; onFlip: (uid: string) => void }) => {
  ...
  return (
    <div className="relative aspect-[3/4] perspective-[1200px] group">
      <motion.div
        onClick={() => onFlip(card.uid)}
        onKeyDown={handleKeyDown}
        tabIndex={card.isMatched ? -1 : 0}
        role="button"
        aria-label={card.isFlipped || card.isMatched ? `Carta virada: ${card.emoji}` : "Carta virada para baixo"}
        animate={{ 
        ...
        className={`w-full h-full relative ${card.isMatched ? "cursor-default" : "cursor-pointer"} outline-none focus-visible:ring-4 focus-visible:ring-emerald-400 rounded-[2rem]`}
...
export default function MemoriaSustentavel({ onExit, onXP }: Props) {
  ...
  useEffect(() => {
    if (matchedCount === totalPairs && !finished) {
      ...
      return () => clearTimeout(timer); // Limpeza de timeout essencial
    }
  }, [matchedCount, totalPairs, onXP, finished]);

  // Usar useCallback evita que a função seja recriada a cada renderização
  const handleFlip = useCallback((uid: string) => {
    ...
    if (newFlippedIds.length === 2) {
      ...
      // Removida a exclamação "!" para segurança de tipos real
  ...
  return (
    <div className="min-h-screen bg-[#022c22] bg-[radial-gradient(ellipse_at_top,_var(--tw-gradient-stops))] from-emerald-900 via-[#064e3b] to-[#022c22] p-4 md:p-8 text-white font-sans overflow-hidden relative">
      ...
      <div className="max-w-4xl mx-auto relative z-10 flex flex-col min-h-[90vh]">
        {/* HUD */}
        <div className="flex flex-col md:flex-row items-center justify-between mb-8 gap-6 bg-white/5 p-5 rounded-[2.5rem] backdrop-blur-xl border border-white/10 shadow-[0_8px_32px_rgba(0,0,0,0.3)]">
          <Button 
            variant="ghost" 
            onClick={onExit} 
        ...
        {/* MENSAGEM */}
        {!finished && (
          <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} className="text-center mb-8 mt-2">
            <h2 className="text-2xl md:text-3xl font-black text-transparent bg-clip-text bg-gradient-to-r from-emerald-200 to-green-400 drop-shadow-[0_2px_15px_rgba(52,211,153,0.3)] tracking-wide uppercase">
              Encontre os pares correspondentes
            </h2>
            <p className="text-emerald-200/70 text-sm md:text-base mt-2 font-medium">
              Combine a causa e efeito de cada elemento sustentável
            </p>
          </motion.div>
        )}
          ...
          <AnimatePresence mode="wait">
            {finished ? (
              ...
              >
                ...
                <motion.div animate={{ rotate: [0, 5, -5, 0] }} transition={{ duration: 4, repeat: Infinity, ease: "easeInOut" }} className="inline-block mb-8 relative">
              ...
              >
                {cards.map((card) => (
                  <MemoryCard key={card.uid} card={card} onFlip={handleFlip} />
                ))}
              </motion.div>
            )}
          </AnimatePresence>
